...
func unsubscribeOp(channel, instType string) (*map[string]interface{}, error) {
...
func loginOp(apiKey string, passphrase string, timestamp string, sign string) (*map[string]interface{}, error) {
...
func (st *SubscriptionTopic) ToString() (topic string, err error) {
	...
	if len(st.instType) > 0 {
		return st.channel + ":" + st.instType, nil
	} else {
