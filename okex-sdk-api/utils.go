...
func NewReqParams() *ReqParams {
	return &ReqParams{params: make(map[string]string)}
}

func (p *ReqParams) AddParam(property string, value string) {
	if property != "" && value != "" {
		p.params[property] = value
	}
	return
}

func (p *ReqParams) Params() map[string]string {
	return p.params
}
